...
object JobRepository {
  ...
  object Failed extends Status
    ...
    final case class Job(id: String, projectName: String, status: Status, duration: Long)
  ...
  final case class GetJobById(id: String, replyTo: ActorRef[Option[Job]]) extends Command
  final case class ClearJobs(replyTo: ActorRef[Response]) extends Command

  // This behavior handles all possible incoming messages and keeps the state in the function parameter
  def apply(jobs: Map[String, Job] = Map.empty): Behavior[Command] = Behaviors.receiveMessage {
